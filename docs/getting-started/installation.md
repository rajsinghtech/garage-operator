...
helm show chart "$CHART" --version 0.7.11

# Resolve the OCI manifest descriptor with an OCI client such as ORAS.
oras manifest fetch --descriptor "$CHART:0.7.11"
...
```bash
gh release download v0.7.11 --repo rajsinghtech/garage-operator \
  --pattern install.yaml
gh attestation verify install.yaml --repo rajsinghtech/garage-operator
```

Substitute the release being installed for `v0.7.11`. Pin verified image and
