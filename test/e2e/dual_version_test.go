...
`, v1Storage, testNS, adminTokenSecret)
		...
		Eventually(func(g Gomega) {
			...
			get := exec.Command("kubectl", "get", "gc.v1beta2.garage.rajsingh.info", v1Storage, "-n", testNS,
...
`, v1RoundtripCluster, testNS, adminTokenSecret)
		...
		get := exec.Command("kubectl", "get", "gc.v1beta2.garage.rajsingh.info", v1RoundtripCluster, "-n", testNS,
		...
		get = exec.Command("kubectl", "get", "gc.v1beta1.garage.rajsingh.info", v1RoundtripCluster, "-n", testNS,
			"-o", "jsonpath={.spec.replicas}")
		out, err = utils.Run(get)
		Expect(err).NotTo(HaveOccurred())
...
`, v1MigrateGateway, testNS, v1MigrateStorage, adminTokenSecret)
	...
	It("supports migrating a two-CR setup to a single v1beta2 unified CR", func() {
		...
		_, err := utils.Run(exec.Command("kubectl", "delete", "gc.v1beta1", v1MigrateGateway, "-n", testNS, "--ignore-not-found"))
		Expect(err).NotTo(HaveOccurred())
...
`, v1MigrateStorage, testNS, adminTokenSecret)
		...
		Eventually(func(g Gomega) {
			out, err := utils.Run(exec.Command("kubectl", "get", "gc.v1beta2", v1MigrateStorage, "-n", testNS,
...
`, v2ScaleCluster, testNS, adminTokenSecret)
		...
		Eventually(func(g Gomega) {
			out, err := utils.Run(exec.Command("kubectl", "get", "gc.v1beta2", v2ScaleCluster, "-n", testNS,
		...
		_, err = utils.Run(exec.Command("kubectl", "patch", "gc.v1beta2", v2ScaleCluster, "-n", testNS,
...
`, v2RotationCluster, testNS, adminTokenSecret)
		...
		Eventually(func(g Gomega) {
			out, err := utils.Run(exec.Command("kubectl", "get", "gc.v1beta2", v2RotationCluster, "-n", testNS,
				"-o", "jsonpath={.status.gatewayReadyReplicas}"))
			g.Expect(err).NotTo(HaveOccurred())
		...
		before, err := utils.Run(exec.Command("kubectl", "get", "gc.v1beta2", v2RotationCluster, "-n", testNS,
	...
	It("annotates the v1beta1 view of a unified v1beta2 CR with the v1beta2-only marker", func() {
		...
		out, err := utils.Run(exec.Command("kubectl", "get", "gc.v1beta1", v2Unified, "-n", testNS,
